...
# Import the data
effdat <- read.csv("effort.csv")
effdat <- effdat[order(effdat$study),]
...
effectSizesAll<- escalc(measure="SMD", #standardized mean difference
                     ...
                     data= effdat)
#(effdat$Ego.Depletion.Mean - effdat$Control.Mean) / effdat$Ego.Depletion.Std.Dev
...
forest(res,
       ilab=cbind(effdat$Control.Mean, effdat$Ego.Depletion.Mean),
       addfit=FALSE, #Don't include polygon because that would include original
       ilab.xpos=c(-0.8,-1.1), #ylim=c(-1, 47) 
       #xlim=c(-10,7)
       #xlim=c(-12,5)
       #xlim=c(-12,8)
       #xlim=c(-5,2.5)
