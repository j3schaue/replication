...
# Import the data
diffdat <- read.csv("difficulty.csv")
diffdat <- diffdat[order(diffdat$study),]
...
effectSizesAll<- escalc(measure="SMD", #standardized mean difference
                     ...
                     data= diffdat)
#(diffdat$Ego.Depletion.Mean - diffdat$Control.Mean) / diffdat$Ego.Depletion.Std.Dev
...
forest(res,
       ilab=cbind(diffdat$Control.Mean, diffdat$Ego.Depletion.Mean),
       addfit=FALSE, #Don't include polygon because that would include original
       ilab.xpos=c(-0.2,-0.5), #ylim=c(-1, 47) 
       #xlim=c(-10,7)
       #xlim=c(-12,5)
       #xlim=c(-12,8)
       #xlim=c(-5,2.5)
