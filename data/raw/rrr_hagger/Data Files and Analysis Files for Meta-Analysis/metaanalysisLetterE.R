...
# Import the data
LetEdat<- read.csv("LetterE.csv")
LetEdat<- RTdat[order(RTdat$study),]
...
effectSizesAll<- escalc(measure="SMD", #standardized mean difference
                     ...
                     data= LetEdat)
#(LetEdat$Ego.Depletion.Mean - LetEdat$Control.Mean) / LetEdat$Ego.Depletion.Std.Dev
...
forest(res,
       ilab=cbind(LetEdat$Ego.Depletion.Mean, LetEdat$Control.Mean),
       addfit=FALSE, #Don't include polygon because that would include original
       ilab.xpos=c(-6.5,-5.75), #ylim=c(-1, 47) 
       #xlim=c(-10,7)
       #xlim=c(-12,5)
       #xlim=c(-12,8)
       #xlim=c(-5,2.5)
