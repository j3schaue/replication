...
# Import the data
frustdat <- read.csv("frustration.csv")
frustdat <- frustdat[order(frustdat$study),]
...
effectSizesAll<- escalc(measure="SMD", #standardized mean difference
                     ...
                     data= frustdat)
#(frustdat$Ego.Depletion.Mean - frustdat$Control.Mean) / frustdat$Ego.Depletion.Std.Dev
...
forest(res,
       ilab=cbind(frustdat$Control.Mean, frustdat$Ego.Depletion.Mean),
       addfit=FALSE, #Don't include polygon because that would include original
       ilab.xpos=c(-0.7,-1), #ylim=c(-1, 47) 
       #xlim=c(-10,7)
       #xlim=c(-12,5)
       #xlim=c(-12,8)
       #xlim=c(-5,2.5)
