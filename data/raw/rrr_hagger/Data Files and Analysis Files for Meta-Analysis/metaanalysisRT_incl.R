...
# Import the data
RTdat<- read.csv("RT_incl.csv")
RTdat<- RTdat[order(RTdat$study),]
...
effectSizesAll<- escalc(measure="SMD", #standardized mean difference
                     ...
                     data= RTdat)
#(RTdat$Ego.Depletion.Mean - RTdat$Control.Mean) / RTdat$Ego.Depletion.Std.Dev
...
forest(res,
       ilab=cbind(RTdat$Control.Mean, RTdat$Ego.Depletion.Mean),
       addfit=FALSE, #Don't include polygon because that would include original
       ilab.xpos=c(-3.3,-3.8), #ylim=c(-1, 47) 
       #xlim=c(-10,7)
       #xlim=c(-12,5)
       #xlim=c(-12,8)
#      xlim=c(-5,2.5)
...
### Create a subset of the data that excludes Sripada et al. data and renumber it
RRRdat <- subset(RTdat,Study != 0)
