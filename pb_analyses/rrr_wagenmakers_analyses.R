...
###------------------------------------------------------------###
###------------------------------------------------------------###
### ANALYSES OF RRR:Wagenmakers REPLICATIONS
###------------------------------------------------------------###
###------------------------------------------------------------###
###------------------------------------------------------------###
...
# load the data
df = read.csv("../data/rrr_wagenmakers.csv") 

###------------------------------------------------------------###
###------------------------------------------------------------###
### RRR:Wagenmakers Comparison
###------------------------------------------------------------###
###------------------------------------------------------------###
...
for(mm in methods){# loop through methods
  ...
  comptab$paper = 'wagenmakers'
  
  write.csv(comptab, paste0("./results/comparison_rrr-wagenmakers_", mm, ".csv"), row.names=F)
}


###------------------------------------------------------------###
###------------------------------------------------------------###
### RRR:Wagenmakers Heterogenetiy Q-Test
###------------------------------------------------------------###
###------------------------------------------------------------###
...
fetab
write.csv(fetab, "./results/qtest_fixed_rrr-wagenmakers_include.csv", row.names=F)
...
fetab
write.csv(fetab, "./results/qtest_fixed_rrr-wagenmakers_exclude.csv", row.names=F)


###------------------------------------------------------------###
###------------------------------------------------------------###
### RRR:Wagenmakers variance components
###------------------------------------------------------------###
###------------------------------------------------------------###
...
for(mm in methods){# for each method, compute tau^2 for each set of replicates
  ...
  tab$vbar = vbars
  tab$paper = 'wagenmakers'
  write.csv(dplyr::select(tab, experiment, tau2=estimate, ci.lb, ci.ub, paper), 
            paste0('./results/rrr-wagenmakers_vc_include_', mm,'.csv'), row.names=F)
}
...
for(mm in methods){# for each method, compute tau^2 for each set of replicates
  ...
  tab$paper = 'wagenmakers'
  write.csv(dplyr::select(tab, experiment, tau2=estimate, ci.lb, ci.ub, paper), 
            paste0('./results/rrr-wagenmakers_vc_exclude_', mm,'.csv'), row.names=F)
}
