...
foo = df_inc %>% group_by(experiment) %>%
  ...
  summarize(nmin = min(n), 
            ...
            nq3 = quantile(n, .75), 
            nmax = max(n)) 
write.csv(foo, "mlsamplesizes_d.csv", row.names=F)
...
for(i in seq(lc)){
  write.csv(lc[[i]],
            paste0("./results/comparison_", paper, "_", names(lc)[i], "_d.csv"), 
          row.names=F)
}
...
fetab_inc
write.csv(fetab_inc, 
          paste0("./results/qtest_fixed_", paper, "_include_d.csv"), row.names=F)
...
write.csv(fetab_inc_ol, 
          paste0("./results/qtest_fixed_", paper, "_include_outlier_d.csv"), 
          row.names=F)
...
fetab_exc
write.csv(fetab_exc, 
          paste0("./results/qtest_fixed_", paper, "_exclude_d.csv"),
          row.names=F)
...
write.csv(fetab_exc_ol, 
          paste0("./results/qtest_fixed_", paper, "_exclude_outlier_d.csv"),
          row.names=F)
...
for(mm in methods){# for each method, compute tau^2 for each set of replicates
  ...
  write.csv(dplyr::select(tab, experiment, tau2=estimate, ci.lb, ci.ub, paper), 
            paste0('./results/manylabs_vc_include_', mm,'_d.csv'), 
            row.names=F)
}
...
for(expt in experiments){
  ...
  dev.copy(pdf, paste0("./results/plots/", paper, "_", expt, "_d.pdf"))
  dev.off()
}
