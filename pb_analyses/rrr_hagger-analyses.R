...
for(mm in methods){# loop through methods
  ...
  comptab$paper = 'hagger'
  
  write.csv(comptab, paste0("./results/comparison_rrr-hagger_", mm, ".csv"), row.names=F)
}
...
for(mm in methods){# for each method, compute tau^2 for each set of replicates
  ...
  tab$vbar = vbars
  tab$paper = 'hagger'
  write.csv(dplyr::select(tab, experiment, tau2=estimate, ci.lb, ci.ub), 
